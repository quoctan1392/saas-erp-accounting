...
CREATE TABLE invoice (
    ...
    -- Invoice Information
    invoice_form VARCHAR(50) NOT NULL,                    -- Mẫu số hóa đơn
    invoice_sign VARCHAR(50) NOT NULL,                    -- Ký hiệu hóa đơn
    invoice_number VARCHAR(50),                           -- Số hóa đơn (auto-generated when published)
    ...
    ref_id UUID,                                          -- ID chứng từ liên quan
    ref_type VARCHAR(50),                                 -- sale_voucher | purchase_voucher | etc
    
    -- Customer/Buyer Information
    account_object_id UUID NOT NULL,                      -- ID đối tượng (khách hàng/người mua)
    account_object_name VARCHAR(255) NOT NULL,            -- Tên đối tượng
    account_object_address TEXT,                          -- Địa chỉ
    account_object_tax_code VARCHAR(50),                  -- Mã số thuế
    identity_number VARCHAR(50),                          -- CMND/CCCD (nếu cá nhân)
    phone_number VARCHAR(20),                             -- Số điện thoại
    
    -- Payment Information
    payment_method VARCHAR(20) NOT NULL DEFAULT 'cash',   -- cash | bank_transfer | both
    CHECK (payment_method IN ('cash', 'bank_transfer', 'both')),
    
    -- Currency Information
    currency_id UUID NOT NULL,                            -- Loại tiền
    exchange_rate DECIMAL(18, 6) NOT NULL DEFAULT 1,      -- Tỷ giá
    
    -- Amounts
    total_amount DECIMAL(18, 2) NOT NULL DEFAULT 0,       -- Tổng tiền hàng
    total_discount_amount DECIMAL(18, 2) NOT NULL DEFAULT 0, -- Tổng chiết khấu
    total_vat_amount DECIMAL(18, 2) NOT NULL DEFAULT 0,   -- Tổng thuế GTGT
    total_payment DECIMAL(18, 2) NOT NULL DEFAULT 0,      -- Tổng thanh toán (sau thuế)
    
    -- Electronic Invoice Provider Info (if applicable)
    einvoice_provider_id UUID,                            -- ID nhà cung cấp hóa đơn điện tử
    einvoice_transaction_id VARCHAR(255),                 -- ID giao dịch từ nhà cung cấp
    einvoice_url TEXT,                                    -- URL hóa đơn điện tử
    
    -- Metadata
    note TEXT,                                            -- Ghi chú
    created_by UUID,                                      -- User tạo
    updated_by UUID,                                      -- User cập nhật
    published_by UUID,                                    -- User phát hành
    published_at TIMESTAMP,                               -- Thời gian phát hành
    cancelled_by UUID,                                    -- User hủy
    ...
    CONSTRAINT uq_invoice_number UNIQUE (tenant_id, invoice_form, invoice_sign, invoice_number)
);
...
CREATE TABLE invoice_detail (
    ...
    -- Item Information
    item_id UUID NOT NULL,                                -- ID hàng hóa
    item_code VARCHAR(50) NOT NULL,                       -- Mã hàng hóa
    ...
    -- Unit Information
    unit_id UUID NOT NULL,                                -- Đơn vị tính
    unit_name VARCHAR(50) NOT NULL,                       -- Tên đơn vị
    
    -- Quantity and Price
    quantity DECIMAL(18, 3) NOT NULL,                     -- Số lượng
    unit_price DECIMAL(18, 2) NOT NULL,                   -- Đơn giá
    amount DECIMAL(18, 2) NOT NULL,                       -- Thành tiền (quantity * unit_price)
    
    -- Discount
    discount_rate DECIMAL(5, 2) DEFAULT 0,                -- % chiết khấu
    discount_amount DECIMAL(18, 2) DEFAULT 0,             -- Tiền chiết khấu
    
    -- VAT
    vat_rate DECIMAL(5, 2) DEFAULT 0,                     -- % thuế GTGT
    vat_amount DECIMAL(18, 2) DEFAULT 0,                  -- Tiền thuế GTGT
    
    -- Total
    total_amount DECIMAL(18, 2) NOT NULL,                 -- Tổng cộng (sau thuế)
    
    -- Display Order
    line_number INTEGER NOT NULL,                         -- Số thứ tự dòng
...
CREATE INDEX idx_invoice_number ON invoice(tenant_id, invoice_number) WHERE invoice_number IS NOT NULL;
...
CREATE POLICY invoice_detail_tenant_isolation ON invoice_detail
    FOR ALL
    USING (
        invoice_id IN (
            SELECT id FROM invoice WHERE tenant_id = current_setting('app.current_tenant_id', TRUE)::UUID
        )
    );
...
CREATE TRIGGER trg_invoice_detail_updated_at
    BEFORE UPDATE ON invoice_detail
    FOR EACH ROW
    EXECUTE FUNCTION update_invoice_updated_at();
