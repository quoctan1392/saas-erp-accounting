...
export class OpeningBalanceController {
  ...
  @Post(':balanceId/details/batch')
  @ApiOperation({ summary: 'Tạo nhiều chi tiết số dư cùng lúc (xóa cũ, tạo mới)' })
  @ApiResponse({
    status: 201,
    description: 'Chi tiết số dư đã được tạo',
    type: [OpeningBalanceDetail],
  ...
  async batchCreateBalanceDetails(
    ...
    @UserId() userId: string,
    @Param('balanceId') balanceId: string,
    @Body() dto: BatchCreateOpeningBalanceDetailsDto,
  ): Promise<OpeningBalanceDetail[]> {
    return this.service.batchCreateBalanceDetails(tenantId, userId, balanceId, dto);
  }
